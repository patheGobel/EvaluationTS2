...
\[
\text{donc } ABDC \text{ est un carré, donc il est inscrit dans un cercle } \mathcal{C}\left(I, \right) .
\]

\[
\text{Tel que les sommets de } ABCD \text{ appartiennent à } \mathcal{C}, \text{ avec } \mathcal{C}(O,1).
\]
...
\text{Or }  I = \text{bar}\{(A,1); (C,-2)\} et J = \text{bar}\{(A,1); (C,2)\}\\
				&\implies -3\overrightarrow{MI}.\overrightarrow{MJ} = 0\\
				&\implies \overrightarrow{MI}.\overrightarrow{MJ} = 0\\
\end{align*}

L'ensemble des points \( M \) est donc un cercle de centre $\frac{z_I+z_J}{2}$ et de rayon $\frac{|z_I-z_J|}{2}$
\[
\mathcal{C} = \{M \in \math{C} \}.
\]
\end{enumerate}
